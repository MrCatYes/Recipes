...
export async function flyersRoutes(app: FastifyInstance) {
  ...
  app.get('/flyers/stats', async () => {
    ...
    const total = await prisma.flyerItem.count();
    const matched = await prisma.flyerItem.count({ where: { productId: { not: null } } });
    const chains = await prisma.flyerItem.groupBy({
      by: ['storeId'],
      _count: true,
    });
    const latestItem = await prisma.flyerItem.findFirst({ orderBy: { createdAt: 'desc' } });
    return {
      totalItems: total,
