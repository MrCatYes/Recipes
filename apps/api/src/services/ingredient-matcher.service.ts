...
function cleanRawText(raw: string): string {
  return raw
    ...
    .replace(/,?\s*(?:au go[uû]t|pour\s+le\s+service|environ|au\s+besoin|à\s+volonté|selon\s+le\s+go[uû]t|pour\s+chaque\s+\w+|par\s+portion|par\s+personne|au\s+moulin|fraîchement\s+(?:moulu|moulue|râpé|râpée|concassé)|pour\s+le\s+d[eé]cor|pour\s+servir)\s*$/gi, '')
